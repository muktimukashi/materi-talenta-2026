# Python Adventure — Versi Full dengan Keterangan Per Part

Dokumen ini berisi **kode lengkap dalam satu file `main.py`**. Di dalam kode terdapat penanda `PART` agar guru dan siswa dapat melihat hubungan setiap bagian kode dengan materi pembelajaran.
...
File `skor.json` akan dibuat otomatis ketika siswa menyelesaikan permainan dengan skor tertinggi baru.
...
# Penjelasan Tiap Part

## Part 1 — Import Library

Library yang digunakan:

- `pygame` untuk layar, bentuk, input keyboard, font, dan collision.
- `json` untuk menyimpan skor.
- `os` untuk memeriksa keberadaan file.

## Part 2 — Konstanta dan Variabel

Bagian ini menyimpan ukuran layar, FPS, nama game, nama file, dan warna. Konstanta ditulis menggunakan huruf kapital agar mudah dikenali.

## Part 3 — File JSON

Fungsi `baca_data_skor()` membaca skor tertinggi. Fungsi `simpan_data_skor()` menyimpan nama dan skor jika pemain membuat rekor baru.

## Part 4 — Class Player

Class `Player` menyimpan:
...
- Skor.
- Nyawa.
- Status kebal sementara.

Method di dalamnya mengatur input, gravitasi, batas layar, reset posisi, collision musuh, dan gambar pemain.

## Part 5 — Class Enemy

Musuh bergerak bolak-balik dengan mengubah nilai `arah` antara `1` dan `-1`.

## Part 6 — Fungsi Tampilan

Fungsi tampilan memisahkan pekerjaan menggambar latar, platform, koin, pintu, HUD, dan teks. Ini membuat game loop lebih mudah dibaca.

## Part 7 — Data Level

Platform, koin, pintu, dan musuh dibuat di fungsi `buat_level()`. Platform dan koin menggunakan struktur data `list`.

## Part 8 — Collision Platform

Fungsi `gerakkan_player()` menangani:

- Gerak horizontal.
- Gravitasi.
- Collision dari atas dan bawah platform.
- Pemain jatuh keluar layar.

## Part 9 — Menu Awal

Menu awal menerima nama pemain melalui keyboard. Siswa belajar event `KEYDOWN`, `BACKSPACE`, `ENTER`, dan pengolahan string.

## Part 10 — Halaman Hasil

Halaman hasil menampilkan kondisi menang atau kalah, skor akhir, dan informasi rekor baru.

## Part 11 — Gameplay Utama

Game loop utama menangani:
...
- Collision pintu.
- Skor dan nyawa.
- Menggambar seluruh objek.

## Part 12 — Program Utama

Fungsi `main()` menghubungkan menu, gameplay, halaman hasil, tombol ulang, dan tombol keluar.
...
| `Space` | Melompat |
| `Enter` | Memulai game |
| `R` | Mengulang permainan |
| `Esc` | Kembali atau keluar |

---

# Hubungan dengan Materi

| Part | Materi utama | Implementasi |
|---:|---|---|
| 1 | Library | Import Pygame, JSON, dan OS |
| 2 | Variabel dan tipe data | Ukuran, warna, teks, FPS |
| 3 | Fungsi dan file | Membaca serta menyimpan JSON |
| 4 | OOP | Class Player |
| 5 | OOP dan percabangan | Class Enemy |
| 6 | Fungsi | Fungsi tampilan |
| 7 | Struktur data | List platform dan koin |
| 8 | Loop dan kondisi | Collision platform |
| 9 | Input pengguna | Nama pemain dan keyboard |
| 10 | Percabangan | Kondisi menang dan kalah |
| 11 | Game loop | Gameplay lengkap |
| 12 | Modularisasi | Fungsi main dan alur aplikasi |
...
|---|---|
| Menutup jendela | Game berhenti tanpa error |
| Gerak kiri-kanan | Pemain tidak keluar layar |
| Lompat | Pemain naik lalu turun |
| Mendarat | Pemain berdiri di atas platform |
| Mengambil koin | Skor bertambah 10 |
| Menyentuh musuh | Nyawa berkurang satu kali |
| Menyentuh musuh saat kebal | Nyawa tidak langsung berkurang lagi |
| Jatuh dari layar | Nyawa berkurang dan posisi di-reset |
| Masuk pintu terkunci | Muncul pesan mengambil semua koin |
| Semua koin terkumpul | Pintu berubah hijau |
| Masuk pintu terbuka | Level selesai |
| Skor tertinggi baru | File `skor.json` diperbarui |
